...
class TsodyksMarkram(bp.dyn.Expon):
  def __init__(self, size, *args, **kwargs):
    ...
    self.tsyn.value = self.tsyn.at[:].set(-np.inf)
    ...
    self.integral_u = odeint(self.derivative_u, method='euler')
  ...
  def add_current(self, weight):
    ...
    t = bp.share['t']
    tsyn = self.tsyn.value
    tau = self.tau
    tau_rec = self.tau_rec
    tau_facil = self.tau_facil
    exp = bm.exp
    ...
    self.z.value = z*active + self.z.value*(~active)
    y = y*exp(-(t - tsyn)/tau)
    self.y.value = y*active + self.y.value*(~active)
    ...
    self.u.value = u*active + self.u.value*(~active)    
      
    self.g.value += (1/self.U) * weight*x*u
    self.y.value += x*u*active

    self.tsyn.value = t*active + tsyn*(~active)
...
class EINet(bp.DynamicalSystem):
  def __init__(self, noise=True, N=1000, b=0.8, c=0.05,_jee=None,_jii=None):
    ...
    # self.NE = PoissonInput(self.E2E.pron.syn.g, int(ne*c), 2, Je*10)
    # self.NI = PoissonInput(self.E2I.pron.syn.g, int(ne*c), 2, Je*10)

    self.NE = bp.dyn.OUProcess(ne,0,4,10)
    self.NI = bp.dyn.OUProcess(ni,0,4,10)

    # self.NE = lambda: ne*c*Je/2000 * 200
    # self.NI = lambda: ne*c*Je/2000 * 200
...
class SynTest(bp.DynamicalSystem):
  def __init__(self,dbs_freq,n_dbs=100):
    super().__init__()
    self.N    = bp.dyn.LifRef(1)
    dbs_isi   = 1000/dbs_freq
    dbs_times = np.arange(n_dbs)*dbs_isi+dbs_isi
    self.IN = bp.dyn.SpikeTimeGroup(1, times=dbs_times, indices=dbs_times*0)
    self.con = Exponential(self.IN, self.N, 1, 1, 1,  2.,  0.)
  def update(self):
    self.con()
    self.IN()
    self.N()
...
# plt.plot(syn_run.mon['ts'],syn_run.mon['N.V'],label='V')
# # plt.plot(syn_run.mon['ts'],syn_run.mon['con.pron.syn.tsyn'],label='tsyn')
# # plt.plot(syn_run.mon['ts'],syn_run.mon['con.pron.syn.g'],label='g')
# # plt.plot(syn_run.mon['ts'],syn_run.mon['con.pron.syn.y'],label='y')
# # plt.plot(syn_run.mon['ts'],syn_run.mon['con.pron.syn.z'],label='z')
# # plt.plot(syn_run.mon['ts'],syn_run.mon['con.pron.syn.u'],label='u')
# plt.legend()
# plt.show()
...
obs1,nostim_max_min = run(
  ({'t_stop':t_stop,'Je_scale':jes,'Ji_scale':jis,
    'dbs_times':np.zeros(1),'progress':True,
    'R_obs':['lfp_max','lfp_min'],
    'aff_scale':affs,'eff_scale':effs,
...
obs3,stim_max_min = run(({'t_stop':t_stop,'Je_scale':jes,'Ji_scale':jis,'dbs_times':dbs_times,'progress':True,'R_obs':['lfp_max','lfp_min'],'aff_scale':affs,'eff_scale':effs,'dbs_pct_aff':0.,'dbs_pct_eff':0.05},{'progress':True}))
...
# plt.show()
# obs4,stim_lfp = run(({'t_stop':t_stop,'Je_scale':jes,'Ji_scale':jis,'dbs_times':dbs_times,'progress':True,'R_obs':['lfp']},{'progress':True}))

# # obs5,stim_max_min_lfp = run(({'t_stop':t_stop,'Je_scale':jes,'Ji_scale':jis,'dbs_times':dbs_times,'progress':True,'R_obs':['lfp_max','lfp_min','lfp']},{'progress':True}))
